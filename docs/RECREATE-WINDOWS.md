...
| Chat replies are **one word** then stop | the system prompt filled `num_ctx`; no room to generate | keep the homelab doc gated (`context.py`), keep `num_ctx` ≥ prompt + reply |
